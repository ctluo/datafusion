"""
DataFusion 算法性能对比评测脚本 —— LTV CN | 按特征范围划分训练/测试集（缩小范围）
- 低置信度数据 : LTV_Low-Fidelity.xlsx   (全量, 1000条)
- 高置信度数据 : LTV_High-Fidelity.xlsx  (按特征范围拆分)
- 训练集筛选   : 1.6 < Mach < 2.8 且 3.0 < Alpha < 17.0
- 测试集       : 其余数据
- 特征列       : Mach, Alpha
- 目标列       : CN
...
TAG         = 'LTV_CN_region_narrow'  # 输出文件前缀

# 训练集特征范围（缩小版）
TRAIN_MACH_MIN = 1.6
TRAIN_MACH_MAX = 2.8
TRAIN_ALPHA_MIN = 3.0
TRAIN_ALPHA_MAX = 17.0
...
print("=" * 65)
print(f"  DataFusion 4-Algorithm Benchmark  [LTV CN | Region Split (Narrow)]")
print("=" * 65)
...
fig.suptitle(
    'DataFusion Algorithm Benchmark  [LTV CN | Region Split (Narrow)]\n'
    f'(LF: {X_lf.shape[0]} samples | '
    f'HF Train: {X_hf_train.shape[0]} pts [1.6<Mach<2.8, 3.0<Alpha<17.0] | '
