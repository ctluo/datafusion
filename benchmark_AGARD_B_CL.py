"""
DataFusion 算法性能对比评测脚本 —— 外形: AGARD-B | 目标变量: CL
- 低置信度数据 : AGARD-B_CL_Low-Fidelity.xlsx   (全量, 23条)
- 高置信度数据 : AGARD-B_CL_High-Fidelity.xlsx  (20% / 50% / 80% 训练)
- 特征列       : Ma, sinA
- 目标列       : CL
...
FEATURES     = ['Ma', 'sinA']
TARGET       = 'CL'
HF_RATIOS    = [0.20, 0.50, 0.80]       # HF 训练集比例
ALGO_COLORS  = ['#2196F3', '#4CAF50', '#FF9800', '#E91E63']
ALGO_NAMES   = ['MF-IDW', 'ConvexHull-GP', 'GPy-CoKriging', 'GPy-InvDistMean']
TAG          = 'AGARD_B_CL'              # 输出文件前缀（注意不能有中划线导致文件名问题）

# ═══════════════════════════════════════════════════════════════════════════════
# 1. 加载数据
# ═══════════════════════════════════════════════════════════════════════════════
df_lf = pd.read_excel(os.path.join(SCRIPT_DIR, 'AGARD-B_CL_Low-Fidelity.xlsx'))
df_hf = pd.read_excel(os.path.join(SCRIPT_DIR, 'AGARD-B_CL_High-Fidelity.xlsx'))
...
print("=" * 65)
print(f"  DataFusion 4-Algorithm Benchmark  [Shape: AGARD-B | Target: {TARGET}]")
print("=" * 65)
...
def evaluate(y_true, y_pred, name):
    ...
    # CL 量级约 0~0.23，取绝对值 > 0.005 的点计算相对误差
    mask  = np.abs(y_true) > 0.005
...
for ratio in HF_RATIOS:
    ...
    for idx, (name, y_pred) in enumerate(preds.items()):
        ...
        resid = y_pred - y_true
        rmse  = np.sqrt(mean_squared_error(y_true, y_pred))
        ax.scatter(y_true, resid, s=55, alpha=0.75,
                   color=ALGO_COLORS[idx], edgecolors='none')
    ...
    fig.suptitle(
        f'DataFusion Algorithm Benchmark  [AGARD-B | Target: {TARGET}]\n'
