...
- 高置信度数据: LTV_High-Fidelity.xlsx (20% 训练 / 80% 测试)
- 特征列: Mach, Alpha
- 目标列: CN
...
# ═══════════════════════════════════════════════════════════════════════════════
print("=" * 65)
print("  DataFusion 4-Algorithm Benchmark  [HF 20% Train / 80% Test]")
print("=" * 65)
...
X_hf_train, X_hf_test, Y_hf_train, Y_hf_test = train_test_split(
    X_hf_all, Y_hf_all, test_size=0.80, random_state=42
)

print(f"\n数据概览:")
print(f"  低置信度训练集   : {X_lf.shape[0]} 条  (全量)")
print(f"  高置信度训练集   : {X_hf_train.shape[0]} 条  (20%)")
print(f"  高置信度测试集   : {X_hf_test.shape[0]} 条  (80%)")
...
fig.suptitle('DataFusion Algorithm Benchmark\n'
             '(LF: 1000 samples | HF Train: 20% | HF Test: 80%)',
             fontsize=13, fontweight='bold', y=0.98)

out_fig = os.path.join(SCRIPT_DIR, 'fusion_benchmark_20pct.png')
...
# 6. 保存结果到 Excel
# ═══════════════════════════════════════════════════════════════════════════════
out_xlsx = os.path.join(SCRIPT_DIR, 'fusion_benchmark_20pct_results.xlsx')
