"""
DataFusion 算法性能对比评测脚本 —— LTV CN | 按特征范围划分训练/测试集
- 低置信度数据 : LTV_Low-Fidelity.xlsx   (全量, 1000条)
- 高置信度数据 : LTV_High-Fidelity.xlsx  (按特征范围拆分)
- 训练集筛选   : 1.2 < Mach < 3.5 且 3.0 < Alpha < 17.0
- 测试集       : 其余数据
- 特征列       : Mach, Alpha
- 目标列       : CN
...
print("=" * 65)
print(f"  DataFusion 4-Algorithm Benchmark  [LTV CN | Region Split]")
print("=" * 65)
...
fig.suptitle(
    'DataFusion Algorithm Benchmark  [LTV CN | Region Split]\n'
    f'(LF: {X_lf.shape[0]} samples | '
    f'HF Train: {X_hf_train.shape[0]} pts [1.2<Mach<3.5, 3.0<Alpha<17.0] | '
