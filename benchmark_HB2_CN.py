"""
DataFusion 算法性能对比评测脚本 —— 外形: HB2 | 目标变量: CN
- 低置信度数据 : HB2_CN_Low-Fidelity.xlsx   (全量, 39条)
- 高置信度数据 : HB2_CN_High-Fidelity.xlsx  (20% / 50% / 80% 训练)
- 特征列       : H, Ma, sinA
- 目标列       : CN
...
# 1. 加载数据
# ═══════════════════════════════════════════════════════════════════════════════
df_lf = pd.read_excel(os.path.join(SCRIPT_DIR, 'HB2_CN_Low-Fidelity.xlsx'))
df_hf = pd.read_excel(os.path.join(SCRIPT_DIR, 'HB2_CN_High-Fidelity.xlsx'))
...
print("=" * 65)
print(f"  DataFusion 4-Algorithm Benchmark  [Shape: HB2 | Target: {TARGET}]")
print("=" * 65)
...
def evaluate(y_true, y_pred, name):
    ...
    # CN 量级约 0~1.0，取绝对值 > 0.02 的点计算相对误差
...
for ratio in HF_RATIOS:
    ...
    for idx, (name, y_pred) in enumerate(preds.items()):
        ...
        resid = y_pred - y_true
        rmse  = np.sqrt(mean_squared_error(y_true, y_pred))
        ax.scatter(y_true, resid, s=45, alpha=0.75,
                   color=ALGO_COLORS[idx], edgecolors='none')
    ...
    fig.suptitle(
        f'DataFusion Algorithm Benchmark  [HB2 | Target: {TARGET}]\n'
