"""
DataFusion 算法性能对比评测脚本 —— 目标变量: CM
- 低置信度数据 : LTV_Low-Fidelity.xlsx  (全量, 1000条)
- 高置信度数据 : LTV_High-Fidelity.xlsx (10% / 20% / 50% / 80% 训练)
- 特征列       : Mach, Alpha
- 目标列       : CM
...
FEATURES  = ['Mach', 'Alpha']
TARGET    = 'CM'
HF_RATIOS = [0.10, 0.20, 0.50, 0.80]   # HF 训练集比例
...
print("=" * 65)
print(f"  DataFusion 4-Algorithm Benchmark  [Target: {TARGET}]")
print("=" * 65)
...
def evaluate(y_true, y_pred, name):
    ...
    mask  = np.abs(y_true) > 0.5        # CM 量级较大，阈值适当放宽
...
# 3. 主循环：遍历4个比例
# ═══════════════════════════════════════════════════════════════════════════════
all_results = {}   # ratio -> DataFrame
all_preds   = {}   # ratio -> {algo: y_pred}
...
# ═══════════════════════════════════════════════════════════════════════════════
# 4. 逐比例生成可视化图（与 CN 版本格式完全一致）
# ═══════════════════════════════════════════════════════════════════════════════
for ratio in HF_RATIOS:
    pct_label  = f"{int(ratio*100)}pct"
    ...
    for idx, (name, y_pred) in enumerate(preds.items()):
        ...
        ax.set_xlabel('True CM', fontsize=9)
    ...
    for idx, (name, y_pred) in enumerate(preds.items()):
        ...
        resid = y_pred - y_true
        rmse  = np.sqrt(mean_squared_error(y_true, y_pred))
        ax.scatter(y_true, resid, s=30, alpha=0.75,
                   color=ALGO_COLORS[idx], edgecolors='none')
    ...
    fig.suptitle(f'DataFusion Algorithm Benchmark  [Target: CM]\n'
                 f'(LF: 1000 samples | HF Train: {train_label} | HF Test: {test_label})',
                 fontsize=13, fontweight='bold', y=0.98)

    out_fig = os.path.join(SCRIPT_DIR, f'CM_benchmark_{pct_label}.png')
    plt.savefig(out_fig, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"\n  图表已保存: {out_fig}")

# ═══════════════════════════════════════════════════════════════════════════════
# 5. 保存汇总 Excel（4个比例各一个 sheet，加一个纵向对比 sheet）
# ═══════════════════════════════════════════════════════════════════════════════
out_xlsx = os.path.join(SCRIPT_DIR, 'CM_benchmark_results.xlsx')
with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
    for ratio in HF_RATIOS:
        ...
        preds     = all_preds[ratio]
        _, y_true = all_tests[ratio]
        X_test, _ = all_tests[ratio]

        # 性能汇总 sheet
        df_res.reset_index().to_excel(
            writer, sheet_name=f'Perf_{pct_label}', index=False)

        # 逐点预测 sheet
        df_detail = pd.DataFrame({
            'Mach': X_test[:, 0], 'Alpha': X_test[:, 1], 'CM_True': y_true
        })
        for name, y_pred in preds.items():
            df_detail[f'CM_{name}']    = y_pred
            df_detail[f'Resid_{name}'] = y_pred - y_true
        df_detail.to_excel(writer, sheet_name=f'Pred_{pct_label}', index=False)
